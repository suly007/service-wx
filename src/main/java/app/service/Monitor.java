...
public class Monitor extends Thread {
    ...
    @Autowired
    public Monitor(@Value("${qywx.corpId}") String corpId,@Value("${qywx.zabbix.agentId}") String agentId, DataService dataService, SimpleDateFormat simpleDateFormat) {
    ...
    public void processChg(Map<String, Stocks> currentInfo) {
        for (int i = 0; i < stockMapListChg.size(); i++) {
            ...
            if (stocks.getChgpercent() < change_min) {
                ...
                Message message = new Message(agentId, msg.toString(), "text", open_id, true);
